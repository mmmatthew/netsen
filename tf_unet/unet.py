...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=3, cost="cross_entropy", cost_kwargs={}, summaries=True, **kwargs):
        ...
        self.mask = tf.reduce_max(self.y, axis=3)

        self.cross_entropy = tf.reduce_sum(tf.multiply(self.mask, cross_entropy(
            tf.reshape(self.y, [-1, n_class]),
            tf.reshape(pixel_wise_softmax_2(logits), [-1, n_class])))) / tf.reduce_sum(self.mask)

        self.predicter = pixel_wise_softmax_2(logits)
        self.correct_pred = tf.equal(tf.argmax(self.predicter, 3), tf.argmax(self.y, 3))
        self.accuracy = tf.reduce_sum(tf.multiply(self.mask, tf.cast(self.correct_pred, tf.float32))) / tf.reduce_sum(
            self.mask)
